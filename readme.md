# SIREG PMK LP3I Makassar
## Setup
- download/clone the git repository from
  - `https://github.com/hansenmakangiras/siregpmk.git`
...
- adjust the `hosts` file and the newly created `Homestead.yaml` in the root of the repo according to your needs. Usually that includes:
  ...
  - add an entry to your host file
    - `[IP] siregpmk.app` (e.g. `192.168.33.111 siregpmk.app`)
...
- adjust `folders` and `sites` mapping (optional; it should be set up correctly by default if you followed the steps above).
  ...
  - the `sites: - map: "[HOSTNAME]"` denotes the hostname that the nginx is looking for to serve content on
    - you _should_ adjust that to the hostname chosen for your hostfile (e.g. `siregpmk.app`) although it not necessary since nginx will even respond to another hostname
...
- start the vagrant box with `vagrant up`, ssh into it with `vagrant ssh`, switch to the project folder (by default, this should be `cd /home/vagrant/Code/siregpmk/`) and install the 
...
You should now be able to open http://siregpmk.app/ in your browser and see the Sireg PMK welcome page :)